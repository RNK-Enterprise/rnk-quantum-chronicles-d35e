import { SRD_MODULE_ID } from "../settings.js";
...
export class EncounterGeneratorDialog extends FormApplication {
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            ...
            template: "systems/rnk-quantum-d35e/templates/apps/encounter-generator-dialog.html",
            width: "auto",
            height: "auto",
            closeOnSubmit: false,
            submitOnClose: false,
    ...
    async getCompendiumTables(){
        ...
        async function compendiumTables(name){
            const pack = game.packs.get(`${SRD_MODULE_ID}.roll-tables`);
...
//Function to get the monsters
    async getMonsters() {
        ...
        let j = 0
        let breakOut = false
        if (grabbedTables.find(t => t.id === val).results.filter(result => result.type != 2) != 0) {
            console.log(grabbedTables.find(t => t.id === val).results);
...
//shove it all together Monsters name and CR + How many we found
        ...
        async function makeMonsters() {//add all the monster data to the Encounter Gen window (in the form of a pretty little template)
            var newMonster = await renderTemplate("systems/rnk-quantum-d35e/templates/apps/encounter-generator-template.html", monsterData);
