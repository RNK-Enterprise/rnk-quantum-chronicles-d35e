...
const CompendiumBrowserClass = HandlebarsApplicationMixin ? HandlebarsApplicationMixin(ApplicationV2) : ApplicationV2;

export class CompendiumBrowser extends CompendiumBrowserClass {
  constructor(...args) {
    super(...args);
  ...
  static PARTS = {
    content: {
      template: "systems/rnk-quantum-d35e/templates/apps/compendium-browser.html"
    }
  };
  ...
  _mapItem(pack, item) {
    const result = {
      ...
      item: {
        ...
        name: item.name,
        type: item.type,
        img: item.img?.replace("systems/D35E/", "systems/rnk-quantum-d35e/"),
        system: item.system || {},
    ...
    this.filters = [
      ...
      },
      {
        ...
        items: Object.values(CONFIG.D35E.weaponTypes).reduce((cur, o) => {
          if (!o || typeof o !== "object") return cur;
          cur = cur.concat((Object.entries(o).filter(i => i[0] && typeof i[0] === "string" && !i[0].startsWith("_")).reduce((arr, i) => {
